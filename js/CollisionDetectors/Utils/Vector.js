export default class Vector {
    ...
    Add(factor) {
        ...
        return new Vector({ x: this.x + f.x, y: this.y + f.y, });
    }
  
    Minus(factor) { const f = typeof factor === "object" ? { x: 0, y: 0, ...factor } : { x: factor, y: factor };
        return new Vector({ x: this.x - f.x, y: this.y - f.y, });
    }
    ...
    Project(line) {
      ...
      return new Vector({ x: line.origin.x + line.direction.x * dotvalue, y: line.origin.y + line.direction.y * dotvalue,});
    }
}